<div class="page">
	<div class="content_header border_box">
		<span class="latest_news vertical_center"> <a>Support</a> &rarr; <i >Ticket view</i></span>
	</div>
    <div class="page-body border_box self_clear">
        ...
        <?php
        $n = 1;
        $ticketID = $_GET['tickets'];

        $query = $conn->prepare("UPDATE PS_Website.dbo.Users_Ticket SET New = 0 WHERE TicketID = ? AND New=1");
        $query->bindParam(1, $ticketID, PDO::PARAM_INT);
        $query->execute();

        $query = $conn->prepare("SELECT * FROM PS_Website.dbo.Users_Ticket WHERE UserUID = ? AND TicketID = ? ORDER BY Row ASC");
        $query->bindParam(1, $UserUID, PDO::PARAM_INT);
        $query->bindParam(2, $ticketID, PDO::PARAM_INT);
        $query->execute();
        ...
        while ($row = $query->fetch(PDO::FETCH_NUM)) {
            ...
            if ($row[3] == NULL) {
                echo '<div class="box">
						...
						<div class="user question">Me</div>
						' . $row[6] . '
					</div>';
            } else {
        ...
        if ($status == 1) {
            echo "<form action='$TemplateUrl/actions/support/contact.php' method='post' >
				<textarea cols='80' id='editor1' name='editor1' rows='10'></textarea>
				<input value='$ticketID' type='hidden' name='TicketID'>
				<input type='hidden' value='0' name='new' id='new'>
				<input type='submit' value='Foward' class='form-submit' style='margin: 10px auto; display: block; width: 150px;'>
			</form>";
        } elseif ($status == 2) {
