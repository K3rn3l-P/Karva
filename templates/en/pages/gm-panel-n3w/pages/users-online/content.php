<div class="page">
    ...
    <div class="page-body border_box self_clear">
        ...
        switch ($order) {
            case 0:
                $order = 'ORDER BY LastIP ASC';
				$switchOrder = 1;
                break;
            case 1:
                $order = 'ORDER BY LastIP DESC';
				$switchOrder = 0;
                break;
            default:
                $order = ' ';
    ...
    <div class="container" style="margin: 60px auto 0; width: 220px;">
        ...
        <table  style='width: 100%; margin-top: 20px;'>
            <tr>
                ...
                <th>Kills</th>
                <th>Deaths</th>
                <th><a href="?p=gm-panel-n3w&sp=users-online&order=<? echo $switchOrder ?>">IP (press for order)</a></th>
            </tr>
            <?php
            $command = $conn->prepare('SELECT c.CharName, c.Job, c.Level, c.Map, c.K1, c.K2, u.Country, c.CharID, c.UserUID, c.UserID, (SELECT TOP 1 LastIP FROM PS_GameLog.dbo.UserLoginStatus WHERE UserUID = c.UserUID) as LastIP
FROM PS_GameData.dbo.Chars c 
JOIN PS_GameData.dbo.UserMaxGrow u ON c.UserUID = u.UserUID 
JOIN PS_UserData.dbo.Users_Master umaster ON c.UserUID = umaster.UserUID
WHERE c.LoginStatus = 1 AND umaster.AdminLevel <= 0' . $condition . $order);
            $command->execute();
