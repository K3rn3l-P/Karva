<?php
$query = $conn->prepare("SELECT UserUID, UserID FROM PS_GameData.dbo.Chars WHERE CharName='$charname' AND Del=0");
$query->execute();
...
$query = $conn->prepare("INSERT INTO [PS_WebSite].[dbo].[AdminLog] ([UserUID],[UserID],[Action],[Text],[IP])
		VALUES ($UserUID, '$UserID', 'Adding item to giftbox by charname', 'ITEM: $itemid; COUNT: $count; UID: $userData[UserUID]; USERNAME: $userData[UserID]', '$UserIP')");
$query->execute();