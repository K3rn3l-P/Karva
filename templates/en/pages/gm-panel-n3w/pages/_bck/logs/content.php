<?php
// Ottenere i valori dai parametri GET in modo sicuro
...
// Costruire le condizioni per la query
$userCondition = $charCondition = $actionCondition = $dateCondition = "1=1";

// Condizione per UserID
if ($userId) {
...
if ($charName) {
    ...
    $charCondition = "AL.CharID=$charId";
}

// Condizione per ActionType
if ($actionTypes) {
    $label = implode(",", $actionTypes);
    $actionCondition = "AL.ActionType IN ($label)";
}

// Condizione per Data
if ($dateStart) {
    $dateStart = (new DateTime($dateStart))->format("Y-m-d H:i:s");
    $dateCondition = "AL.ActionTime >= '$dateStart'";
    if ($dateEnd) {
        $dateEnd = (new DateTime($dateEnd))->format("Y-m-d H:i:s");
        $dateCondition .= " AND AL.ActionTime BETWEEN '$dateStart' AND '$dateEnd'";
    }
} elseif ($dateEnd) {
    $dateEnd = (new DateTime($dateEnd))->format("Y-m-d H:i:s");
    $dateCondition = "AL.ActionTime <= '$dateEnd'";
}

// Combinazione delle condizioni
$conditions = "WHERE $userCondition AND $charCondition AND $actionCondition AND $dateCondition";

// Costruzione della query
$query = "SELECT TOP $rows AL.*, UM.AdminLevel 
