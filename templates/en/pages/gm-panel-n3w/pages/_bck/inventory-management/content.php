...
$userUid = 0;
if (!$itemUid && $charName) {
	$SqlRes = odbc_exec($odbcConn, "SELECT CharID FROM PS_GameData.dbo.Chars WHERE CharName='{$charName}' ORDER BY Del");
	$charId = odbc_num_rows($SqlRes) ? odbc_result($SqlRes, "CharID") : 0;	
} elseif (!$itemUid && $userId) {
	$SqlRes = odbc_exec($odbcConn, "SELECT UserUID FROM PS_UserData.dbo.Users_Master WHERE UserID='{$userId}'");
	$userUid = odbc_num_rows($SqlRes) ? odbc_result($SqlRes, "UserUID") : 0;	
}
