...
<?php
// Qui inizia la parte per la ricerca delle inclusioni di file
$file = __FILE__; // Il percorso del file corrente

// Apri il file corrente per la lettura
$handle = fopen($file, "r");

// Leggi il file linea per linea
while (($line = fgets($handle)) !== false) {
    // Verifica se la linea contiene una chiamata a include, require, include_once o require_once
    if (preg_match('/\b(include|require)(_once)?\s*\(.*\)/', $line, $matches)) {
        // Estrai il nome del file incluso dalla stringa di inclusione
        preg_match('/(["\'])(.*?)\1/', $matches[0], $fileMatches);
        $includedFile = $fileMatches[2];

        // Stampa il percorso del file incluso
        echo "Il file $includedFile è incluso in $file\n";
    }
}
