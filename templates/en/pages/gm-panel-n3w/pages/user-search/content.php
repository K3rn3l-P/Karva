    ...
    <div class="page-body border_box self_clear">
		...
		<form method="GET">
			...
			<span class="orange" style="width: 17%; display: inline-block;">CharID: </span><input type="text" name="CharID"/>
			 <br />
			<span class="orange" style="width: 17%; display: inline-block;">IP: </span><input type="text" name="UserIP" placeholder="e.g search by range: 88.166.%.%"/>
		...
		if (isset($_GET["UserID"]) && !empty($_GET["UserID"])) {
			$val = $_GET["UserID"];
			$query = "SELECT * FROM PS_UserData.dbo.Users_Master WHERE UserID=? AND Status <= 0";
			// Log action				
			...
			$val = $_GET["UserUID"];
			$query = "SELECT * FROM PS_UserData.dbo.Users_Master WHERE UserUID=? AND Status <= 0";
			// Log action				
			odbc_exec($odbcConn, "INSERT INTO [PS_WebSite].[dbo].[AdminLog] ([UserUID],[UserID],[Action],[Text],[IP])
					VALUES ($UserUID, '$UserID', '[User-Search] By UserUID', 'VALUE: $val', '$UserIP')");
		} elseif (isset($_GET["CharID"]) && !empty($_GET["CharID"])) {
			$val = $_GET["CharID"];
			$query = "SELECT * FROM PS_UserData.dbo.Users_Master WHERE UserUID=(SELECT TOP 1 UserUID FROM PS_GameData.dbo.Chars WHERE CharID=?) AND Status <= 0";
			// Log action				
			odbc_exec($odbcConn, "INSERT INTO [PS_WebSite].[dbo].[AdminLog] ([UserUID],[UserID],[Action],[Text],[IP])
					VALUES ($UserUID, '$UserID', '[User-Search] By CharID', 'VALUE: $val', '$UserIP')");
		} elseif (isset($_GET["CharName"]) && !empty($_GET["CharName"])) {
			$val = $_GET["CharName"];
			$query = "SELECT * FROM PS_UserData.dbo.Users_Master WHERE UserUID=(SELECT TOP 1 UserUID FROM PS_GameData.dbo.Chars WHERE CharName=? ORDER BY Del) AND Status <= 0";
			// Log action				
			odbc_exec($odbcConn, "INSERT INTO [PS_WebSite].[dbo].[AdminLog] ([UserUID],[UserID],[Action],[Text],[IP])
					VALUES ($UserUID, '$UserID', '[User-Search] By CharName', 'VALUE: $val', '$UserIP')");
		} elseif (isset($_GET["UserIP"]) && !empty($_GET["UserIP"])) {
			$val = $_GET["UserIP"];
			$query = "SELECT * FROM PS_UserData.dbo.Users_Master WHERE UserUID in (SELECT DISTINCT UserUID FROM PS_GameLog.dbo.ActionLog WHERE text1 like ? AND actiontype= 107) AND Status <= 0";
			// Log action				
			odbc_exec($odbcConn, "INSERT INTO [PS_WebSite].[dbo].[AdminLog] ([UserUID],[UserID],[Action],[Text],[IP])
