<div class="page">
    ...
    <div class="page-body border_box self_clear">
		...
		if (isset($_POST["UserID"]) && !empty($_POST["UserID"])) {
			...
			$val = $_POST["CharName"];
			$query = "SELECT UserUID, UserID FROM PS_UserData.dbo.Users_Master WHERE UserUID=(SELECT TOP 1 UserUID FROM PS_GameData.dbo.Chars WHERE CharName=? ORDER BY Del) AND Status <= 0";
		} elseif (isset($_POST["IP"]) && !empty($_POST["IP"])) {
			$val = $_POST["IP"];
			$query = "SELECT UserUID, UserID FROM PS_UserData.dbo.Users_Master WHERE UserUID IN (SELECT DISTINCT(UserUid) FROM PS_GameLog.dbo.UserLoginStatus WHERE LastIP = ?) AND Status <= 0";
		}
				...
				<table>
					...
					<tr>
						...
						<td><select name="days">
								...
								<option value="8">Temporarily banned</option>
							</select></td>
					</tr>
					<tr>
						<td>Reason:</td>
		...
		while ($charAccount2 = $queryAccount2->fetch(PDO::FETCH_NUM)) {
			...
			echo "<tr><td>" . $charAccount1[0] . "</td><td>" . $charAccount2[2] . "</td><td>" . $charAccount2[3] . "</td><td>" . $charAccount2[4] . "</td><td>" . $charAccount2[5] . "</td> </tr>";
		}
		echo "</table>";
		?>
		<!-- end content -->	
